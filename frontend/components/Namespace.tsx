...
import { DocNode, groupNodes, cleanJSDoc } from "../util/docs";

export const Namespace = (props: { nodes: DocNode[]; name: string }) => {
  const groups = groupNodes(props.nodes);

  return (
    <div className="p-8">
      <div className="text-gray-900 text-3xl font-medium">{props.name}</div>

      {groups.classes.length > 0 ? (
        <div className="py-4">
          <div className="text-gray-900 text-2xl font-medium mb-1">Classes</div>
          <div>
            {groups.classes.map(node => (
              <div className="py-2">
                <p>
                  <Link href={`/${node.kind}/${node.name}`}>
